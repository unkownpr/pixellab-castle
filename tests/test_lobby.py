...
def test_lobby_status_never_exposes_raw_pairing_or_controller_secrets(
    ...
    slot = service.lobby_status(session.admin_token, clock.now)["slots"][0]

    assert grant.code not in slot.values()
    assert claimed.controller_token not in slot.values()
    assert "pairing_digest" not in slot
    assert "capability_digest" not in slot
...
def test_slot_configuration_cannot_orphan_an_issued_capability(
    ...
    assert service.observe(claimed.controller_token).colony_id == "c1"
